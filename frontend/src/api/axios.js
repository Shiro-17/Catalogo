...
export const apiChat = axios.create({ baseURL: 'http://localhost:3004/api/chat' });
const addToken = (config) => {
  const token = localStorage.getItem('token');
  if (token) {
    config.headers.Authorization = `Bearer ${token}`; // Así el backend sabe quién eres
  }
  return config;
};
...
apiChat.interceptors.request.use(addToken);